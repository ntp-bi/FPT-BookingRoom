...
const Room = () => {
    ...
    const roomTypes = [...new Set(rooms.map((room) => room.typeName))];
    ...
    useEffect(() => {
        const fetchApi = async () => {
            setLoadingSearch(true);
             const result = await api.searchRoom(debouncedValue); // Gọi API tìm kiếm
    ...
    const filterRows = (selectedRoomTypes, selectedStatus, searchTerm) => {
        const filteredRows = rooms.filter((room) => {
            const roomTypeMatch =
                selectedRoomTypes.length === 0 || selectedRoomTypes.includes(room.typeName);
    ...
    const handleDelete = async (id) => {
        ...
        if (confirmed) {
            try {
                ...
                toast.error(`Phòng đang được sử dụng!`);
            }
        }
        setTimeout(() => {
            toast.dismiss();
                ...
                <div className="roomList">
                    ...
                    <div className="roomSearch">
                        ...
                        >
                            ...
                            {roomTypes.map((type) => (
                                <option key={type} value={type.typeName}>
                                    {type}
                                </option>
                            ))}
                        </select>
