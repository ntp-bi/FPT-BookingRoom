...
const List = () => {
    ...
    useEffect(() => {
        const fetchApi = async () => {
            ...
            const result = await api.searchList(debouncedValue); // Gọi API tìm kiếm
    ...
    const handleSearch = () => {
        filterRows(search, selectedDateRange);
    };

    const handleDateRangeChange = (newDateRange) => {
        setSelectedDateRange(newDateRange);
        filterRows(search, newDateRange);
    };
    ...
    const filterRows = (searchTerm, dateRange) => {
        const [startDate, endDate] = dateRange;

        const filteredList = lists.filter((list) => {
            const timeMatch =
                (!startDate ||
                    dayjs(list.reserveTime).isAfter(startDate) ||
                    dayjs(list.reserveTime).isSame(startDate, "day")) &&
                (!endDate ||
                    dayjs(list.endTime).isBefore(endDate) ||
                    dayjs(list.endTime).isSame(endDate, "day"));
                    ...
                    <div className="listSearch">
                        ...
                        <div className="search">
                            <input
                                ref={inputRef}
                                spellCheck={false}
                                placeholder="Nhập họ tên khách hàng muốn tìm."
                                value={search}
