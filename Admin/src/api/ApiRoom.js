...
export async function searchRoom(roomName) {
    try {
        const token = localStorage.getItem("token");

        const result = await api.get(`/room/both/searchroom?roomname=${roomName}`, {
            headers: {
                Authorization: `Bearer ${token}`,
            },
        });
...
export async function bookRoom({ roomId, userId, eventId, reserveTime, endTime }) {
    try {
        const token = localStorage.getItem("token");
